...
class ControllerCatalogProduct extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		if (isset($this->request->post['description_image'])) {
			$data['description_image'] = $this->request->post['description_image'];
		} elseif (!empty($product_info)) {
			$data['description_image'] = $product_info['description_image'];
		} else {
		...
		if (isset($this->request->post['coordinates'])) {
			$data['coordinates'] = $this->request->post['coordinates'];
		} elseif (!empty($product_info)) {
			$data['coordinates'] = $product_info['coordinates'];
		} else {
			$data['coordinates'] = '';
		}
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
		...
		//Sliders
		if (isset($this->request->post['product_slider'])) {
			$product_sliders = $this->request->post['product_slider'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_sliders = $this->model_catalog_product->getProductSlider($this->request->get['product_id']);
		} else {
			$product_sliders = array();
		}

		$data['product_sliders'] = array();

		foreach ($product_sliders as $product_slider) {
			if (is_file(DIR_IMAGE . $product_slider['slider'])) {
				$image = $product_slider['slider'];
				$thumb = $product_slider['slider'];
			} else {
				$image = '';
				$thumb = 'no_image.png';
			}
			...
			$data['product_sliders'][] = array(
				'slider'      => $image,
				'thumb'       => $this->model_tool_image->resize($thumb, 100, 100),
				'description' => $product_slider['description']
			);
		}
