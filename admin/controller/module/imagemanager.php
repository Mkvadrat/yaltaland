...
class ControllerModuleImagemanager extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->url->link('module/imagemanager', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		
		if (isset($this->request->post['imagemanager_language'])) {
			$data['imagemanager_language'] = $this->request->post['imagemanager_language'];
		} else {
			$data['imagemanager_language'] = $this->config->get('imagemanager_language');
		}
		
		if($this->config->get('imagemanager_language')){
			$data['current_language'] = $this->config->get('imagemanager_language');
		}
		...
		$this->response->setOutput($this->load->view('module/imagemanager.tpl', $data));
	}
