...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				$data['banner'] = $this->model_tool_image->resize($product_info['image'], '1025', '400');
				$this->document->setOgImage($data['banner']);
			} else {
				$data['banner'] = $this->model_tool_image->resize('no_banner.jpg', '1025', '400');
			}
			...
			$get_slider = $this->model_catalog_product->getProductSlider($this->request->get['product_id']);
			
			foreach ($get_slider as $result) {
				$data['sliders'][] = array(
					'full' 		  => $this->model_tool_image->resize($result['slider'], '924', '472'),
